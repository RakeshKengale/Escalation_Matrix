...
if(isset($_POST['hubupdate']))
{
$did=intval($_GET['hubid']);  
$hub=$_POST['hub'];  
$locationname=$_POST['locationname'];

   
$sql="update tblhub set Hubname=:hub,Locationid=:locationname where id=:did";
$query = $dbh->prepare($sql);
$query->bindParam(':hub',$hub,PDO::PARAM_STR);
$query->bindParam(':locationname',$locationname,PDO::PARAM_STR);
$query->bindParam(':did',$did,PDO::PARAM_STR);
$query->execute();
   echo"<script>alert('Hub Updated Successfully');document.location = 'hub.php';</script>";
}
}
    ?>
<!doctype html>
                     ...
                     <div class="col-lg-12" style="padding-bottom: 1%; padding-top:1%; padding-left: 5%; padding-right: 5%;">
                         <div class="col-md-6 col-lg-12">
                           <form  method="post">
						    <?php 
                              $did=intval($_GET['hubid']);
                              $sql = "SELECT * from tblhub WHERE id=:did";
                              $query = $dbh -> prepare($sql);
                              $query->bindParam(':did',$did,PDO::PARAM_STR);
                              $query->execute();
                              ...
                              <div class="form-group">
                                 ...
                                 <select class="form-control" name="locationname" autocomplete="off">
                                    ...
                                    <?php $sql = "SELECT * FROM tbllocation";
                                       ...
                                       {   ?>                                            
                                    <option value="<?php echo htmlentities($resultt->Locationid);?>"> <?php echo htmlentities($resultt->Locationname);?></option>
                                    <?php }} ?>
                                 </select>
                              </div>
                              <div class="form-group">
                                 <label>Hub</label>
                                 <input type="text" class="form-control" name="hub" value="<?php echo htmlentities($result->Hubname); ?>" placeholder="Enter Location" required>
								 <?php }} ?>
                              </div>
                              <hr />
                              <div class="container">
                                 <button type="submit" name="hubupdate" class="btn btn-success btn-lg">Submit</button>
                              </div>
                           </form>
                        </div>
                     </div>
